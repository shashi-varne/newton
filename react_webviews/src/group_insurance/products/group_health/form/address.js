...
class GroupHealthPlanAddressDetails extends Component {
    ...
    onload = async () => {
        ...
        if (this.state.provider === 'HDFCERGO' && form_data.pincode && form_data.pincode.length === 6 && permanent_address.city) {
            ...
            let errorType='';
            try {
                var url = this.getApiUrl(`api/insurancev2/api/insurance/proposal/hdfc_ergo/validate_pincode?pincode=${pincode}&city=${cityName}`)
    ...
    getCityListReligare = async ({form_data={}, name = ''}) => {
        ...
        var url = this.getApiUrl(`api/insurancev2/api/insurance/proposal/religare/validate_pincode?pincode=${form_data[name]}`)
        const res = await Api.get(url);
        this.setState({isLoadingCity: false});
