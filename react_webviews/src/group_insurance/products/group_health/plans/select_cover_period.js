...
class GroupHealthPlanSelectCoverPeriod extends Component {
  ...
  async componentDidMount() {
    ...
    if (
      this.state.groupHealthPlanData.account_type === "self" ||
      Object.keys(this.state.groupHealthPlanData.post_body.member_details)
        .length === 1
    ) {
          ...
          () => {
            this.updateBottomPremium(
              this.state.premium_data[this.state.selectedIndex].premium ||
                this.state.premium_data[0].premium
            );
          }
        );
      } else {
  ...
  handleClick = async () => {
    ...
    for (var addOn in plan_selected_final.add_on_premium) {
      if (
        addOn !== "total" &&
        plan_selected_final.add_on_premium[addOn] !== 0
      ) {
        if (addOn === "opd") {
    ...
    groupHealthPlanData.post_body.add_on_premium =
      plan_selected_final.add_on_premium.total;
      ...
      >
        <div className="select-tile">
          <div className="flex-column">
            <div className="name">
              {props.tenure} year{props.tenure !== 1 && <span>s</span>} for{" "}
              {inrFormatDecimal(props.premium)}
            </div>
            {this.state.type_of_plan === "floater" &&
              props.discount.tenure[1] > 0 && (
                <div className="flex" style={{ margin: "4px 0 0 0" }}>
                  <img
                    style={{ width: 10 }}
                    src={require(`assets/ic_discount.svg`)}
                    alt=""
                  />
                  <span
                    style={{
                  ...
                  >
                    save {inrFormatDecimal(props.discount.tenure[1])}{" "}
                  </span>
                </div>
              )}
            {this.state.type_of_plan === "non_floater" &&
              props.discount.tenure[1] > 0 && (
                <div className="flex" style={{ margin: "4px 0 0 0" }}>
                  <img
                    style={{ width: 10 }}
