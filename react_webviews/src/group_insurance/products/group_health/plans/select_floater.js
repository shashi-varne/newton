...
class GroupHealthPlanSelectFloater extends Component {
    ...
    async componentDidMount() {
        ...
        if(post_body.mem_info.child > 0){
            for(var key in post_body){
                    if(key.includes('child') && calculateAge(post_body[key].dob, false) < 5 && this.state.providerConfig.key == "RELIGARE"){
    ...
    choosePlan = (index, props) => {
        if(props.key === "NF" && this.state.disableFloaterOption && this.state.providerConfig.key == "RELIGARE"){
            return;
        }
        this.setState({
            selectedIndex: index
    ...
    renderPlans = (props, index) => {
        ...
        return (
            ...
            <div onClick={() => this.choosePlan(index, props)}
                className={`tile ${index === this.state.selectedIndex ? 'tile-selected' : ''} ${this.state.disableFloaterOption && props.key === "NF" && this.state.providerConfig.key == "RELIGARE" ? 'tile-disabled': ''}`} key={index}>
