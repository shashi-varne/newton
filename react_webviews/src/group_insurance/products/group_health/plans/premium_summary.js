...
class GroupHealthPlanPremiumSummary extends Component {
  ...
  onload = async () => {
    ...
    if(!this.state.get_lead){
      ...
      try{
      const url = this.getApiUrl(`api/insurancev2/api/insurance/health/quotation/upsert_quote/${this.state.providerConfig.provider_api}`)
  ...
  handleClick = async () => {
    ...
    let { groupHealthPlanData } = this.state;
      try {
        ...
        const url = this.getApiUrl(  `api/insurancev2/api/insurance/proposal/${this.state.providerConfig.provider_api}/create_application`)
