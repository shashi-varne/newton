...
class GroupHealthPlanList extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      const res = await Api.post(
        `api/insurancev2/api/insurance/health/quotation/plans/${this.state.providerConfig.provider_api}`,
        body
      );
      ...
      properties: {
        ...
        plan_type:
          this.state.provider === "HDFCERGO"
            ? this.state.providerConfig.hdfc_plan_title_mapper[plan.plan_id]
            : plan.plan_name,
  ...
  selectPlan = (plan, index) => {
    ...
    if (provider === "HDFCERGO") {
      groupHealthPlanData.plan_selected.plan_title = this.state.providerConfig.hdfc_plan_title_mapper[
        plan.plan_id
      ];
    }
  ...
  renderPlans = (props, index) => {
    ...
    return (
      <div
        className="tile"
        key={index}
        onClick={() => this.selectPlan(props, index)}
      >
        <div
          className="group-health-recommendation"
          style={{
            backgroundColor:
              props.recommedation_tag === "Recommended" ? "#E86364" : "",
          }}
        >
          {plan_data.recommedation_tag}
        </div>
        <div className="group-health-top-content-plan-logo">
          <div className="left">
            ...
            <div className="tc-subtitle">
              {this.state.provider === "HDFCERGO"
                ? this.state.providerConfig.hdfc_plan_title_mapper[
                    props.plan_id
                  ]
                : plan_data.plan_name}
            </div>
    ...
    return (
      ...
      >
        <div className="group-health-plan-list">
          <div className="tiles">
            {this.state.plan_data.plans &&
              this.state.plan_data.plans.map(this.renderPlans)}
          </div>
        </div>
      </Container>
    );
