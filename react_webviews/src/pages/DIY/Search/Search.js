...
const Search = (props) => {
  ...
  return (
    <ContainerWrapper
      noFooter
      headerProps={{
        headerTitle: iframe ? (isMobileDevice ? 'Where do you want to invest?' : '') : 'Search',
      }}
          ...
          <div className='search-content' data-aid='search-content'>
            <div className='search-option'>
              <div className='search-input' data-aid='search-input'>
                <input placeholder='Fund Search...' value={value} onChange={handleChange} />
                {value && value.length !== 0 && (
                  <Close className='close-icon' onClick={clearInputFields} />
                )}
              </div>
              <div
                data-aid='search-btn'
        ...
        {!showLoader && fundResult && (
          <>
            ...
            {fundResult.length === 0 && showNoFundmessage && (
              <div className='message' data-aid='diy-message'>
                No result found
              </div>
            )}
          </>
