...
import {
  isEquityCompleted,
  isUpgradeToEquityAccountEnabled,
...
export async function initialize({
  ...
  if (
    ["investLanding", "sdkLanding"].includes(screenName) &&
    dataSettedInsideBoot
  ) {
    storageService().set("dataSettedInsideBoot", false);
  }
  const isBfdlBannerDisplayed = storageService().getBoolean(
    "bfdlBannerDisplayed"
  );
  const isBfdlConfig =
    !isBfdlBannerDisplayed &&
    config.code === "bfdlmobile" &&
    (config.isIframe || config.isSdk);
...
export function getInvestCardsData() {
  ...
  const disabledPartnersMap = {
    insurance: ["cccb", "sury", "obc", "svcho", "alb", "ktb", "sbm", "cub"],
...
export async function getRecommendationsAndNavigate({
  amount,
  handleLoader,
  navigate,
}) {
...
export const getKycData = (userKyc, userData) => {
  ...
  let kycJourneyStatusMapperData = kycJourneyStatus?.includes("ground_")
    ? kycStatusMapper["incomplete"]
    : kycStatusMapper[kycJourneyStatus];
  const isKycCompleted =
    (TRADING_ENABLED && isEquityCompletedBase) ||
    (!TRADING_ENABLED && isReadyToInvestBase);
  const showKycCard =
    !isKycCompleted ||
    (TRADING_ENABLED &&
...
export const initializeKyc = ({
  ...
  const { isCompliant, kycJourneyStatus, tradingEnabled } = kycData;
  const contactDetails = contactVerification(
    kyc,
    handleDialogStates,
    screenName
  );
  ...
  if (isCompliant && !tradingEnabled) {
    ...
    let isLandingBottomSheetDisplayed = storageService().get(
...
export async function setKycProductTypeAndRedirect({
  ...
  if (
    isReadyToInvestBase &&
    (result?.kyc?.mf_kyc_processed || kyc?.mf_kyc_processed)
  ) {
...
function initiatePinSetup(
  { key, isDirectEntry, navigate, handleLoader, handleSummaryData },
  props
) {
  const config = getConfig();
  if (config.isNative) {
  ...
  if (key === "ipo") {
    ...
    if (kycJourneyStatus === "verifying_trading_account") {
      modalData = {
        ...modalData,
        subtitle:
          "This process could take upto 48 hours. We will notify you once it’s done. Meanwhile, you can explore primary market products",
...
export const handleStocksRedirection = ({
  isDirectEntry = false,
  navigate,
}) => {
  if (isDirectEntry) {
...
export const handleKycPremiumLanding =
  ({ screenName, modalData, navigate, handleDialogStates }) =>
  () => {
    if (
...
export const handleKycStatusRedirection =
  ...
  () => {
    ...
    const { contactValue, communicationType, contactNotVerified } =
      contactDetails;
...
export function getSdkLandingCardsData({ kyc, user }) {
  ...
  const hideReferral =
    user.active_investment && !isWeb && config?.referralConfig?.shareRefferal;
  const referralCode =
    !user.active_investment && !isWeb && config?.referralConfig?.applyRefferal;
...
export function getCampaignData() {
  ...
  const campaignData = notifications.reduceRight((acc, data) => {
    ...
    if (target?.length >= 1) {
      ...
      target.some((el, idx) => {
        if (
          el?.view_type === "bottom_sheet_dialog" &&
          el?.section === "landing"
        ) {
          acc = el;
...
export function contactVerification(kyc, handleDialogStates, screenName) {
  ...
  const isVerifyDetailsSheetDisplayed = storageService().get(
  ...
  if (!isVerifyDetailsSheetDisplayed && screenName === "investLanding") {
    if (!isEmpty(contactDetails)) {
      ...
      if (
        ...
        contactDetails.mobile_number_verified === false
      ) {
        contact_type = "mobile";
        isVerified = false;
        contact_value = splitMobileNumberFromContryCode(
          contactDetails?.mobile_number
        );
      } else if (
        !isEmpty(contactDetails.email) &&
        contactDetails.email_verified === false
      ) {
        contact_type = "email";
...
export const handleCampaign =
  ({ campaignData, handleLoader, handleDialogStates }) =>
  () => {
...
export const closeCampaignDialog =
  ({ campaignData, handleDialogStates }) =>
  () => {
...
export async function updateBank(data) {
  ...
  if (response.pfwstatus_code !== 200 || isEmpty(response.pfwresponse)) {
...
export async function handleWealthdeskRedirection(handleLoader) {
  ...
  try {
    const response = await Api.get(
      "/api/equity/api/eqm/get/wealthdesk/redirection/url"
    );
    if (response.pfwstatus_code !== 200 || isEmpty(response.pfwresponse)) {
      throw new Error(response?.pfwmessage || errorMessage);
    }
