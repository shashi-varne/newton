...
export function handleCampaignRedirection (url) {
  ...
  // eslint-disable-next-line
  campLink = `${campLink}${campLink.match(/[\?]/g) ? "&" : "?"}generic_callback=true&plutus_redirect_url=${encodeURIComponent(`${getBasePath()}/${config.searchParams}&is_secure=${storageService().get("is_secure")}`)}`
