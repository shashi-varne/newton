          ...
          <>
            <div className="bank-info">
              ...
              <div className="content">
                ...
                <div className="branch-name" id='branch_name'>{bank.branch_name}</div>
              </div>
            </div>
            <div className="item">
              <div className="left" id='account_number_title'>Account number</div>
              <div className="right" id='account_number'> {bank.account_number} </div>
            </div>
            <div className="item">
              <div className="left" id='ifsc_code_title'>IFSC code</div>
              <div className="right" id='ifsc_code'>{bank.ifsc_code} </div>
            </div>
            <div className="item">
              <div className="left" id='account_type_text'>Account type</div>
              <div className="right" id='account_type'> {bank.account_type} </div>
            </div>
            <div className="item">
              <div className="left" id='Status-text'>Status</div>
              <div
                className={`status ${
                  bank.bank_status === "rejected" && "failed"
                } ${bank.bank_status === "verified" && "verified"}`}
                id={`mapped_bank_status`}
              >
            ...
            {bank.bank_status !== "rejected" && (
              <div className="mandate-section">
                <div className="title" id='Mandates-text'>Mandates</div>
                {bank.mandates && bank.mandates.length > 0 ? (
                  bank.mandates.map((mandate, index) => {
                    return (
                      <div key={index} className="content">
                        <div className="item">
                          <div className="left" id='ID-text'>ID</div>
                          <div className="right" id='ID'>{mandate.id} </div>
                        </div>
                        <div className="item">
                          <div className="left" id='account_type_text'>Account type</div>
                          <div className="right" id='amount'>
                            {" "}
