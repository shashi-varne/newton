...
const Home = (props) => {
  ...
  const initialize = () => {
    ...
    const tradingEnabled = isTradingEnabled(!kyc.address?.meta_data?.is_nri);
    ...
    let data = {
      ...
      title: "Verify PAN",
      subtitle: tradingEnabled ? "As per SEBI, valid PAN is mandatory to open a trading & demat account" : "As per SEBI, valid PAN is required to invest in mutual funds",
