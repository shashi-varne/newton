...
class DocumentUpload extends Component {
  ...
  renderHtmlCamera(type) {
    let { image_data, totalUpload } = this.state;
        ...
        {!image_data[type] && (
          ...
          >
            ...
            <div style={{ margin: "20px 0 20px 0", cursor: "pointer" }}>
              <div
                onClick={() => this.startUpload((method_name || "open_gallery"), type)}
                style={{
                  textAlign: "center",
                  cursor: "pointer",
                }}
              >
                <input
                  type="file"
                  style={{ display: "none" }}
                  onChange={totalUpload === "3" ? this.getPdf : this.getPhoto}
                  id={type ? type : "myFile"}
                />
                ...
                <div key={index}>
                  ...
                  >
                    <div className="sub-title">
                      ...
                      <span
                        style={{ float: "right" }}
                        onClick={() => this.editDocument(index, item)}
                      >  
                        <img
                          // style={{
              ...
              {documents.length < 3 && (
                <div className="upload-bank-statement">
                  <div
                    ...
                    style={{ marginBottom: "50px" }}
                  >
                    {getConfig().html_camera && this.renderHtmlCamera(`doc${documents.length + 1}`)}
                    {!getConfig().html_camera && this.renderNativeCamera(`doc${documents.length + 1}`,)}
                  </div>
                </div>
              )}
            </div>
